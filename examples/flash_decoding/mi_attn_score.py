...
def _fwd_kernel_varlen(
    ...
    stride_sb, stride_sh, stride_sn, #bmask shape [b, q_h, seq/BLOCK_N]
    ...
    tl.store(O_ptrs + offs_h[:, None] * stride_oh + offs_d[None, :] * stride_od, acc,
            mask=mask_h[:, None])
...
def flashattn(batch, heads, k_heads, max_seqlen_kv, total_seqlen_k, dim, has_sink, block_N, block_H, num_split, num_stages,
    ...
    valid_block_N = min(block_N, max_seqlen_kv // num_split)
...
def speed_benchmark_decode_comparison(args):
    ...
    if args.test_sink:
        sink = torch.randn(q_heads, device='cuda', dtype=torch.float32) * 0.1  # Small sink values
        print(f"  Using sink attention with sink values")
