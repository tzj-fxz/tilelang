...
def prepare_input(
    ...
    G = torch.randn(B, S, H, dtype=output_dtype).cuda()
    return K, Beta, G
...
def tilelang_chunk_scaled_dot_kkt_fwd(
    ...
    def kernel(
        ...
        G: T.Tensor(G_shape, dtype=output_dtype),
        ...
        with T.Kernel(T.ceildiv(S, block_S), B * H, threads=threads) as (bs, bbh):
            ...
            A_fragment = T.alloc_fragment((block_S, block_S), dtype=output_dtype)
            
            # Tensor used for gated:
            G_shared = T.alloc_shared((block_S,), dtype=output_dtype, scope="shared")
            G_diff_local = T.alloc_fragment((block_S, block_S), dtype=output_dtype)
...
def run_test(
    ...
    K, Beta, G = prepare_input(B, S, H, DK, getattr(torch, input_dtype), getattr(torch, output_dtype))
    ...
    # tilelang
    block_S = chunk_size
    program = tilelang_chunk_scaled_dot_kkt_fwd(B, S, H, DK, chunk_size, input_dtype, output_dtype, use_g, block_S, block_DK, threads, num_stages)
...
if __name__ == "__main__":
    ...
    run_test(B=1, S=32768, H=32, DK=128, chunk_size=64, input_dtype="float16", output_dtype="float32", use_g=True, block_DK=64, threads=128, num_stages=2)
