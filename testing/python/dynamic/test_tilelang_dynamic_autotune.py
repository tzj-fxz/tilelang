...
def benchmark_tl_matmul_block_static(
    ...
    if not autotune:
        ...
        for autotune_config in autotune_configs:
            ...
            prim_func = tl_matmul_block_static(M, N, K, block_M, block_N, block_K, trans_A, trans_B, in_dtype, out_dtype, accum_dtype, num_stages, thread_num, enable_rasteration)
        ...
        autotuner = AutoTuner.from_kernel(
            kernel=tl_matmul_block_static, configs=autotune_configs).set_compile_args(
                out_idx=[-1],
    ...
    kernel_path = os.path.join(kernel_save_dir, f"best_config_static_{M}_{N}_{K}.cu")
...
def benchmark_tl_matmul_block_all_dynamic(  
    ...
    if not autotune:
        ...
        for autotune_config in autotune_configs:
            ...
            prim_func = tl_matmul_block_all_dynamic(block_M, block_N, block_K, trans_A, trans_B, in_dtype, out_dtype, accum_dtype, num_stages, thread_num, enable_rasteration)
        ...
        # Use Autotuner to find the best config
        autotuner = AutoTuner.from_kernel(
            kernel=tl_matmul_block_all_dynamic, configs=autotune_configs).set_compile_args(
                out_idx=[-1],
    ...
    kernel_path = os.path.join(kernel_save_dir, f"best_config_dynamic_{M}_{N}_{K}.cu")
    with open(kernel_path, "w") as f:
        f.write(kernel.get_kernel_source())
...
def enumerate_configs_tl_matmul_block(M, N, K, with_roller=True, autotune=False, is_static=False):
    autotune_configs = []
...
if __name__ == "__main__":
    ...
    kernel_save_dir = os.path.join("testing/python/dynamic/.log", f"{'static' if is_static else 'dynamic'}_{current_time}")
    ...
    if file_path is None:
        ...
        if M < 0 or N < 0 or K < 0:
            ...
            for MNK_config in MNK_configs:
                df_best_config_list.append(enumerate_configs_tl_matmul_block(MNK_config[0], MNK_config[1], MNK_config[2], with_roller, autotune, is_static))
        else:
            print(f"Running test case with M={M}, N={N}, K={K}")
            df_best_config_list.append(enumerate_configs_tl_matmul_block(M, N, K, with_roller, autotune, is_static))
        df_best_config = pd.DataFrame(df_best_config_list)
        df_best_config.to_csv(os.path.join(kernel_save_dir, f"best_config_{'static' if is_static else 'dynamic'}_{M}_{N}_{K}.csv"), index=False)

    else:
        df = pd.read_csv(file_path)
        output_path = os.path.join(kernel_save_dir, f"best_config_{'static' if is_static else 'dynamic'}_matmul_config_all.csv")
